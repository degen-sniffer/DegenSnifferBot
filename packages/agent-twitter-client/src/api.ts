...
export async function requestApi<T>(
  ...
  do {
    try {
      await new Promise((resolve) => setTimeout(resolve, 1000)); // wait 2 seconds before each request
    ...
    if (res.status === 429) {
      ...
      */
      const xRateLimit = res.headers.get("x-rate-limit-limit");
      const xRateLimitRemaining = res.headers.get("x-rate-limit-remaining");
      const xRateLimitReset = res.headers.get("x-rate-limit-reset");
      console.log(
        `x-rate-limit-limit: ${xRateLimit}, x-rate-limit-remaining: ${xRateLimitRemaining}, x-rate-limit-reset: ${xRateLimitReset}`
      );
